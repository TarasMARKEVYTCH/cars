...
class Car extends Model
{
    ...
    {
        $cars = $this->pdo->query("SELECT * from $this->table order by id DESC limit $start, $nbrPerPage");
        $cars = $cars->fetchAll();
        if (count($cars) == 0) {
    ...
    {
        $updateInfo = $this->pdo->prepare("UPDATE {$this->table} SET name = ?, model = ?, engine = ?, year = ?, gearbox = ?, doors = ?, seats = ?, luggage = ?, description = ?, access = ? where id = ?");
    ...
    {
        $updatePhoto = $this->pdo->prepare("UPDATE {$this->table} SET img = ? where id = ?");
